...
from planning import canonicalize_query_plan
...
def retrieve(question: str, k: int = 12, max_per_source: int = 4) -> List[Dict[str, Any]]:
    ...
    query_plan = canonicalize_query_plan(query_plan, question)
    ...
    for chunk in INDEX:
        ...
        working_chunk = dict(chunk)

        text = working_chunk.get("text", "")
        text_l = text.lower()
        ...
        if doctrine_line == "unknown":
            doctrine_line = working_chunk.get("doctrine_line") or "unknown"

        # Hard doctrine corrections. These override stale index metadata.
        if any(k in source_l for k in ["barkan", "disney"]):
        ...
        if "doctrine entire fairness" in source_l or "entire fairness" in source_l:
            doctrine_line = "entire_fairness"

        role = get_case_role(source)

        if not role or role == "unknown":
            role = working_chunk.get("role", "related_case")
        ...
        chunk_role = working_chunk.get("chunk_role", "")
        quality_score = working_chunk.get("quality_score", 100)
        corrupt = working_chunk.get("corrupt", False)
        ...
        elif doctrine_line == "demand_futility":
            if primary_issue != "demand_futility":
                score *= 0.42
                score -= 0.22
        ...
        if primary_issue == "controller_transactions":
            if doctrine_line in {
                ...
                score *= 1.75
                score += 0.50
            elif doctrine_line == "sale_of_control" and "sale_of_control" in target_set:
                score *= 1.22
                score += 0.16
            elif doctrine_line in {"takeover_defense", "entire_fairness"}:
                score *= 0.50
            elif doctrine_line == "unknown":
                score *= 0.42
        ...
        if is_food_safety_red_flags:
            ...
            if (
                doctrine_line == "takeover_defense"
                and "takeover_defense" not in target_set
            ):
                score *= 0.15
    ...
    # Pure single-doctrine locks only.
    if query_type == "comparison" and target_set == {"oversight"}:
        scored = [
            c for c in scored
            if c.get("doctrine_line") == "oversight"
        ]

    if primary_issue == "disclosure_loyalty" or target_set == {"disclosure_loyalty"}:
        scored = [
            c for c in scored
    ...
    def add_chunk(chunk: Dict[str, Any]) -> bool:
        ...
        role = chunk.get("role") or get_case_role(source)
    ...
    if query_type == "comparison" and target_set == {"oversight"}:
        guaranteed_doctrines = ["oversight"]

    if primary_issue == "disclosure_loyalty" or target_set == {"disclosure_loyalty"}:
        guaranteed_doctrines = ["disclosure_loyalty"]

    # Never guarantee demand-futility unless directly requested.
    if "demand_futility" not in target_set:
        guaranteed_doctrines = [
            d for d in guaranteed_doctrines
            if d != "demand_futility"
        ]
    ...
    # -------------------------------------------------
    # Hard doctrine isolation
    # -------------------------------------------------

    # Demand-futility should never leak unless explicitly requested.
    if "demand_futility" not in target_set:
        allowed_fill_doctrines.discard("demand_futility")

    # Controller / sale-process prompts must stay isolated.
    if (
        "controller_transactions" in target_set
        or "sale_of_control" in target_set
    ):
        allowed_fill_doctrines = {
            x for x in allowed_fill_doctrines
            if x in {
                "controller_transactions",
                "sale_of_control",
                "stockholder_vote_cleansing",
    ...
    # Pure disclosure prompts stay pure.
    if target_set == {"disclosure_loyalty"}:
        allowed_fill_doctrines = {"disclosure_loyalty"}

    # Pure oversight prompts stay pure.
    if target_set == {"oversight"}:
        allowed_fill_doctrines = {"oversight"}

    # Pure demand-futility prompts stay pure.
    if target_set == {"demand_futility"}:
        allowed_fill_doctrines = {"demand_futility"}
    ...
    for chunk in scored:
        ...
        if (
            doctrine_line == "demand_futility"
            and "demand_futility" not in target_set
        ):
            continue

        if allowed_fill_doctrines and doctrine_line not in allowed_fill_doctrines:
            continue
