...
from validation import validate_ai_answer as run_validate_ai_answer
...
def infer_target_lines(question: str) -> List[str]:
    ...
    doctrine_term_map: Dict[str, List[str]] = {
        ...
        "takeover_defense": [
            ...
            "defensive measures", "defense", "takeover defense",
            "poison pill", "hostile bid",
            "coercive", "preclusive",
...
def build_role_based_quote_map(
    ...
    for role in ["foundation", "supreme_refinement", "refinement", "modern_application"]:
        for case in cases:
            ...
            if debug:
                print(f"DEBUG PICKED QUOTE FOR {source}: {best_quote[:220] if best_quote else ''}")
...
def build_required_quote_fragments_block(role_quote_map, target_lines):
    ...
    if not lines:
        lines.append('- Caremark: "utter failure to attempt to assure"')
        lines.append('- Stone: "failure to act in good faith"')
...
High
"""
        if {"oversight", "controller_transactions"} <= target_set:
            return """Short Answer:
<one sentence>
...
High
"""
        if {"oversight", "stockholder_vote_cleansing"} <= target_set:
            return """Short Answer:
<one sentence>
...
"""
        ...
        labels = [line.replace("_", " ").title() for line in target_lines if line != "unknown"]
        if len(labels) >= 2:
...
def synthesize_multi_doctrine_key_distinction(target_lines: List[str]) -> str:
    target_set = set(line for line in target_lines if line != "unknown")

    # --- Caremark vs Revlon ---
    if target_set == {"oversight", "sale_of_control"}:
        return "Caremark governs oversight, whereas Revlon governs sale-of-control duties."

    if target_set == {"oversight", "takeover_defense"}:
        return "Caremark governs oversight, whereas Unocal governs takeover defenses."
    
    # --- Controller vs Sale of Control ---
    if target_set == {"controller_transactions", "sale_of_control"}:
        return "Controller Transactions doctrine governs conflicted controller deals under MFW, whereas Revlon governs value maximization in a sale-of-control context."

    if target_set == {"controller_transactions", "stockholder_vote_cleansing"}:
        return "Controller Transactions doctrine governs conflicted controller deals, whereas Stockholder Vote Cleansing doctrine governs the effect of a fully informed and uncoerced stockholder vote."

    if target_set == {"controller_transactions", "sale_of_control"}:
        return "Controller Transactions doctrine governs conflicted controller deals, whereas Sale of Control doctrine governs value maximization in a change-of-control transaction."
    # --- Fallback (SAFE) ---
    labels = [line.replace("_", " ").title() for line in target_lines if line != "unknown"]

    if len(labels) >= 2:
        return f"{labels[0]} governs one fiduciary context, whereas {labels[1]} governs a distinct one."

    return "The applicable doctrine depends on the fiduciary context."
...
def synthesize_rule_from_quotes(
    ...
    if "oversight" in target_lines:
        return (
        "A board breaches the duty of loyalty where its utter failure to attempt to assure that a reasonable reporting or information system exists, "
        "or its conscious failure to monitor such a system, constitutes a failure to act in good faith; "
        "directors therefore must make a good faith effort to implement an oversight system and monitor it."
    )

    return "Directors breach the duty of loyalty where their conduct constitutes a failure to act in good faith."
...
def synthesize_multi_doctrine_rule_comparison(target_lines: List[str]) -> str:
    ...
    if target_set == {"controller_transactions", "stockholder_vote_cleansing"}:
        return (
            "Controller Transactions doctrine under MFW governs conflicted controller deals through dual procedural protections. "
            "Stockholder Vote Cleansing doctrine under Corwin governs the effect of a fully informed and uncoerced stockholder vote. "
            "Taken together, controller-transactions doctrine addresses conflicted controller deals, whereas stockholder-vote cleansing addresses the legal effect of stockholder approval."
        )

    if target_set == {"controller_transactions", "sale_of_control"}:
        return (
            "Controller Transactions doctrine under MFW governs conflicted controller deals through procedural protections. "
            "Sale of Control doctrine under Revlon governs the board's obligation to secure the best value reasonably available in a change-of-control transaction. "
            "Taken together, controller-transactions doctrine addresses conflict cleansing, whereas sale-of-control doctrine addresses transactional value maximization."
        )

    if target_set == {"oversight", "takeover_defense"}:
        return (
            "Caremark governs board-level monitoring duties in good faith. "
            "Unocal governs defensive responses to a perceived threat to corporate policy and effectiveness. "
            "Taken together, oversight doctrine addresses internal monitoring, whereas takeover-defense doctrine addresses external defensive measures."
        )

    if target_set == {"oversight", "sale_of_control"}:
        return (
            "Caremark governs board-level monitoring duties in good faith. "
            "Revlon governs value maximization in a sale-of-control transaction. "
            "Taken together, oversight doctrine addresses internal monitoring, whereas sale-of-control doctrine addresses transactional value maximization."
        )

    if target_set == {"demand_futility", "oversight"}:
        return (
            "Demand-futility doctrine governs whether a majority of the board can impartially consider a litigation demand. "
            "Oversight doctrine governs whether directors discharged board-level monitoring obligations in good faith. "
            "Taken together, demand-futility doctrine addresses board capacity, whereas oversight doctrine addresses the underlying fiduciary conduct."
        )
    ...
    return (
        ...
        "Each doctrine governs its own category of conduct under its own standard. "
...
def synthesize_multi_doctrine_analysis(target_lines: List[str]) -> str:
    ...
    # 🔥 GLOBAL CAREMARK GUARD
    if "oversight" not in target_set:
        def strip_caremark(text: str) -> str:
            return (
                text
                .replace("utter failure to attempt to assure", "")
                .replace("good faith effort to implement an oversight system", "")
                .replace("conscious failure to monitor", "")
            )
    if target_set == {"controller_transactions", "stockholder_vote_cleansing"}:
        return (
        "This matters because controller-transactions doctrine under MFW requires dual procedural protections to cleanse conflicted controller deals. "
        "The significance is that stockholder-vote cleansing under Corwin focuses on whether a fully informed and uncoerced stockholder vote restores business judgment review. "
    ...
    if target_set == {"controller_transactions", "sale_of_control"}:
        return (
        "This matters because controller-transactions doctrine under MFW requires procedural protections such as an independent special committee and a majority-of-the-minority vote to cleanse conflicts. "
        "The significance is that sale-of-control doctrine under Revlon instead imposes a duty on directors to act reasonably to secure the best value reasonably available in a change-of-control transaction. "
        "As a result, controller-transactions doctrine governs conflict-cleansing in controller deals, whereas sale-of-control doctrine governs value maximization in a sale process."
    )

    if target_set == {"oversight", "controller_transactions"}:
        return (
            "This matters because oversight doctrine governs whether directors discharged their board-level monitoring obligations in good faith. "
            "The significance is that controller-transactions doctrine under MFW instead governs conflicted controller deals through procedural protections such as a special committee and a majority-of-the-minority vote. "
            "As a result, oversight doctrine addresses internal monitoring obligations, whereas controller-transactions doctrine addresses conflict-cleansing in controller transactions."
        )

    if target_set == {"oversight", "stockholder_vote_cleansing"}:
        return (
            "This matters because oversight doctrine governs whether directors discharged their board-level monitoring obligations in good faith. "
            "The significance is that stockholder-vote cleansing under Corwin instead asks whether a fully informed and uncoerced stockholder vote restores business judgment review. "
            "As a result, oversight doctrine addresses internal monitoring obligations, whereas stockholder-vote cleansing addresses the legal effect of stockholder approval."
        )

    if target_set == {"oversight", "takeover_defense"}:
        return (
            "This matters because oversight doctrine governs whether directors discharged their board-level monitoring obligations in good faith. "
            "The significance is that takeover-defense doctrine under Unocal instead evaluates whether defensive measures are coercive, preclusive, and within a range of reasonableness in response to a threat. "
            "As a result, oversight doctrine addresses internal monitoring obligations, whereas takeover-defense doctrine addresses defensive responses to takeover threats."
        )

    if target_set == {"oversight", "sale_of_control"}:
        return (
            "This matters because oversight doctrine governs whether directors discharged their board-level monitoring obligations in good faith. "
            "The significance is that sale-of-control doctrine under Revlon instead focuses on the board's obligation to secure the best value reasonably available in a change-of-control setting. "
            "As a result, oversight doctrine addresses internal monitoring obligations, whereas sale-of-control doctrine addresses transactional value maximization."
        )
    ...
    if target_set == {"demand_futility", "oversight"}:
        return (
            ...
            "The significance is that oversight doctrine instead governs whether directors discharged their board-level monitoring obligations in good faith. "
    ...
    if len(labels) >= 2:
     analysis = (
        ...
        f"The significance is that each doctrine applies a different standard to a different form of board conduct. "
...
# 🔥 APPLY CAREMARK GUARD ONLY HERE
    if "oversight" not in target_set:
        analysis = (
        analysis
        .replace("utter failure to attempt to assure", "")
...
def synthesize_key_distinction(
    ...
    if target_set == {"demand_futility"}:
        return (
            "Aronson governs demand futility through reasonable doubt about director independence or business judgment, whereas Rales governs whether a majority of the board can impartially consider a demand."
        )
...
def synthesize_rule_comparison(
    ...
    target_set = set(x for x in (target_lines or []) if x != "unknown")
    ...
    if target_set == {"demand_futility"}:
        return (
            "Aronson asks whether particularized facts create a reasonable doubt that the directors are disinterested and independent or that the challenged transaction was a valid exercise of business judgment. "
            "Rales asks whether particularized facts create a reasonable doubt that a majority of the board could impartially consider a litigation demand. "
            "Taken together, Aronson governs demand futility where a board decision is under review, whereas Rales governs demand futility where the board did not make the challenged decision or cannot impartially consider demand."
        )

    return (
        "The governing comparison depends on the doctrinal framework implicated by the question. "
        "Different Delaware doctrines apply different standards to different fiduciary settings. "
        "Taken together, the controlling comparison must be drawn from the doctrine actually implicated by the question."
    )
...
def synthesize_analysis_from_tree_and_quotes(
    ...
    if "oversight" in target_lines:
        ...
        if primary_failure == "red_flags" or "mission_critical_red_flags" in path:
            return (
                "This matters because oversight liability turns on whether directors made a good faith effort to monitor risks central to the company's mission. "
                "The significance is that failure to act in good faith, including ignoring red flags or failing to ensure a reporting system, implicates the duty of loyalty. "
                "As a result, a board's failure to respond to red flags in a mission critical setting supports an inference of bad-faith oversight failure."
            )

        if primary_failure == "failure_to_monitor":
            return (
                "This matters because oversight liability may arise where directors consciously fail to monitor an existing reporting or information system in good faith. "
                "The significance is that failure to act in good faith in the monitoring of corporate risk implicates the duty of loyalty even where some formal compliance structure exists. "
                "As a result, the theory sounds in monitoring failure rather than an utter failure to attempt to assure that a reasonable reporting system exists."
            )
...
def upsert_section(answer: str, section_name: str, new_body: str) -> str:
    # If section exists → replace it
    if re.search(rf"(?m)^\s*{re.escape(section_name)}\s*:", answer):
        return replace_section(answer, section_name, new_body)

    # Otherwise → insert before Confidence
    confidence_match = re.search(r"(?m)^\s*Confidence\s*:", answer)
    block = f"{section_name}:\n{new_body.strip()}\n\n"

    if confidence_match:
        idx = confidence_match.start()
        return answer[:idx] + block + answer[idx:]
...
def relock_answer_sections(
    ...
    ai_answer = upsert_section(ai_answer, "Rule", locked_rule)
...
def infer_caremark_facts_from_question(question: str) -> Dict[str, bool]:
    ...
    return {
        "has_reporting_system": not any(phrase in q for phrase in [
            "no reporting system", "no compliance system", "no oversight system",
            "no controls", "utter failure", "never implemented",
        ]),
       "monitors_reporting_system": not any(phrase in q for phrase in [
        ...
        "red_flags_ignored": any(phrase in q for phrase in [
            "ignored red flags", "red flags", "warnings ignored", "board ignored warnings",
        ]),
    }
...
def evaluate_caremark_tree(facts: Dict[str, bool]) -> Dict[str, Any]:
    ...
    # 1. Pure implementation failure: no reporting system at all
    if not has_reporting_system:
        ...
        result["reason"] = (
            "The facts suggest an utter failure to attempt to assure a reasonable "
    ...
    # 2. Most specific oversight failure: mission-critical red flags
    # This must come before generic no-monitoring logic.
    ...
    # 4. System exists and appears to be monitored
    result["path"].append("monitoring_exists")
...
def validate_rule_comparison_v2(
    ...
    print("USING NEW VALIDATE_RULE_COMPARISON_V2")
    errors: List[str] = []
    delta = 0
...
def strip_oversight_fragments(text: str, target_lines: List[str]) -> str:
    if "oversight" in target_lines:
        return text
    ...
    return (
        ...
        .replace("utter failure to attempt to assure", "")
        .replace("good faith effort to implement an oversight system", "")
        .replace("failure to act in good faith", "")
    )
...
def main() -> None:
    ...
    if len(sys.argv) > 1:
        ...
        question = input("Ask a Delaware law question: ")
    ...
    cases,
    case_quotes,
    debug=oversight_debug,
)

    if oversight_debug:
        print("DEBUG ROLE QUOTE MAP:")
    for role, item in role_quote_map.items():
        print(f"{role} -> {item['quote']}")

    if "oversight" in query_plan.get("target_lines", []):
        print("DEBUG NORMALIZED QUOTE FRAGMENTS:")
    for role, item in role_quote_map.items():
        print(role, "->", normalize_quote_fragment(role, item.get("quote", "")))
        
    # ------------------------------------------------------------
    # Optional Caremark tree (single-doctrine oversight only)
    # ------------------------------------------------------------
    tree_result: Optional[Dict[str, Any]] = None
    tree_summary = ""
    ...
    # Context assembly
    # ------------------------------------------------------------
    print("DEBUG QUERY TYPE:", query_plan.get("query_type"))
    print("DEBUG MULTI DOCTRINE:", query_plan.get("multi_doctrine"))
    print("DEBUG TARGET LINES:", query_plan.get("target_lines", []))
    if query_plan.get("multi_doctrine"):
    ...
    required_quote_fragments_block = build_required_quote_fragments_block(
        role_quote_map,
        query_plan.get("target_lines", []),
    )

    if "oversight" in query_plan.get("target_lines", []) and required_quote_fragments_block:
    ...
    locked_analysis = ""
    print("DEBUG LOCKED RULE BEFORE RELOCK:", repr(locked_rule))
    ...
    if query_plan.get("multi_doctrine"):
        ...
        locked_key_distinction = enforce_one_sentence(
            synthesize_multi_doctrine_key_distinction(target_lines)
        )

        locked_rule_a = synthesize_doctrine_section(
            line_a,
            doctrine_a_cases,
            case_quotes,
        )
        locked_rule_b = synthesize_doctrine_section(
            line_b,
            doctrine_b_cases,
            case_quotes,
        )
...
""".strip()
        ...
        analysis_tree_result = tree_result
...
Query Type:
{query_plan.get("query_type")}

Target Doctrine Lines:
{query_plan.get("target_lines", [])}

Named Sources:
{query_plan.get("named_sources", [])}
...
"""
    ...
    ai_answer = (getattr(response, "output_text", "") or "").strip()
    ...
    # ------------------------------------------------------------
    # Relock sections after generation
    # ------------------------------------------------------------
    if query_plan.get("multi_doctrine"):
        ai_answer = replace_section(ai_answer, "Short Answer", locked_short_answer)
        ai_answer = replace_section(ai_answer, "Key Distinction", locked_key_distinction)
        ...
        print("DEBUG LOCKED RULE:")
        print(repr(locked_rule))
    else:
        ai_answer = replace_section(ai_answer, "Short Answer", locked_short_answer)
        ai_answer = replace_section(ai_answer, "Key Distinction", locked_key_distinction)
        ...
        print("DEBUG RULE AFTER RELOCK:")
        print(repr(extract_sections(ai_answer, query_plan).get("rule", "")))

    if query_plan.get("query_type") != "comparison":
        ai_answer = remove_section(ai_answer, "Rule Comparison")

    ai_answer = strip_oversight_fragments(  
    ai_answer,
    query_plan.get("target_lines", [])
)
    ...
    print("DEBUG AI ANSWER AFTER SANITIZATION:")
    print(ai_answer)

    is_valid, validation_errors, validation_score = run_validate_ai_answer(
        ai_answer,
        query_plan,
        role_quote_map,
