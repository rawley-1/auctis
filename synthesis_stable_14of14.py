...
def synthesize_key_distinction(target_lines: List[str]) -> str:
    real_lines = [x for x in target_lines if x and x != "unknown"]
    target_set = set(real_lines)

    # -------------------------------------------------
    # Exact doctrine hard locks
    # -------------------------------------------------

    if target_set == {"oversight"}:
        return (
            "Caremark doctrine addresses utter-failure oversight liability, "
            "whereas Marchand doctrine addresses mission-critical monitoring obligations."
        )

    if target_set == {"controller_transactions", "stockholder_vote_cleansing"}:
        return (
            "Controller Transactions doctrine addresses controller-conflict cleansing protections, "
            "whereas Stockholder Vote Cleansing doctrine addresses the legal effect of a fully informed stockholder vote."
        )

    if target_set == {"controller_transactions", "sale_of_control"}:
        return (
            "Controller Transactions doctrine addresses controller conflict, "
            "whereas Sale of Control doctrine addresses value maximization in a change-of-control transaction."
        )

    if target_set == {"oversight", "sale_of_control"}:
        return (
            "Oversight doctrine addresses board-level monitoring failures, "
            "whereas Sale of Control doctrine addresses value maximization in a change-of-control transaction."
        )

    if target_set == {"oversight", "takeover_defense"}:
        return (
            "Oversight doctrine addresses internal monitoring failures, "
            "whereas Takeover Defense doctrine addresses defensive measures responding to external threats."
        )

    if target_set == {"demand_futility"}:
        return (
            "Aronson addresses demand futility in one procedural posture, "
            "whereas Rales addresses demand futility where the challenged board did not make the business decision."
        )

    # -------------------------------------------------
    # Single-doctrine fallback
    # -------------------------------------------------

    if len(real_lines) < 2:
        line = real_lines[0] if real_lines else "unknown"
        ...
        return f"{label} identifies the governing fiduciary standard."

    # -------------------------------------------------
    # Generic two-doctrine fallback
    # -------------------------------------------------
    ...
    if concept_a and concept_b:
        return (
            f"{label_a} doctrine addresses {concept_a}, "
            f"whereas {label_b} doctrine addresses {concept_b}."
        )

    return (
        f"{label_a} doctrine governs one fiduciary setting, "
        f"whereas {label_b} doctrine governs a distinct one."
    )
...
def synthesize_rule_from_quotes(role_quote_map, target_lines=None):
    ...
    if "enhanced scrutiny" in joined or "coercive" in joined or "preclusive" in joined:
        return (
            ...
            "that is not coercive or preclusive and falls within a range of reasonableness."
        )
...
def synthesize_multi_doctrine_rule_comparison(target_lines: List[str]) -> str:
    ...
    # -------------------------------------------------
    # Exact doctrine-pair hard locks
    # -------------------------------------------------

    if target_set == {"oversight"}:
        return (
        "Caremark establishes the baseline oversight rule requiring an utter failure to attempt to assure that a reasonable reporting or information system exists. "
        "Marchand further requires directors to make a good faith effort to implement and monitor systems addressing mission-critical risk. "
        "Taken together, Caremark defines the foundational oversight framework, whereas Marchand extends that framework into active mission-critical monitoring obligations."
    )

    if target_set == {"controller_transactions", "stockholder_vote_cleansing"}:
        return (
            "Controller Transactions doctrine subjects conflicted controller transactions to entire fairness review absent dual procedural protections. "
            "Stockholder Vote Cleansing doctrine separately asks whether a fully informed and uncoerced stockholder vote restores business judgment review for a non-controller transaction. "
            "Taken together, controller-transactions doctrine addresses controller conflict, whereas stockholder-vote cleansing addresses the legal effect of stockholder approval."
        )

    if target_set == {"controller_transactions", "sale_of_control"}:
        return (
            "Controller Transactions doctrine governs conflicted-controller transactions and focuses on procedural cleansing protections. "
            "Sale of Control doctrine separately governs change-of-control transactions and requires directors to maximize stockholder value. "
            "Taken together, controller-transactions doctrine addresses controller conflict, whereas sale-of-control doctrine addresses value maximization."
        )
    ...
    if target_set == {"oversight", "sale_of_control"}:
        return (
            ...
            "Taken together, oversight doctrine addresses internal monitoring failure, whereas sale-of-control doctrine addresses value maximization."
        )

    if target_set == {"oversight", "takeover_defense"}:
        return (
            "Oversight doctrine governs board-level monitoring obligations and focuses on whether directors acted in bad faith by failing to implement or monitor reporting systems. "
            "Takeover Defense doctrine governs defensive measures adopted in response to a perceived threat and requires that the response be neither coercive nor preclusive and fall within a range of reasonableness. "
    ...
    if target_set == {"demand_futility", "oversight"}:
        return (
            "Demand Futility doctrine governs whether stockholders may proceed derivatively without making a litigation demand. "
            "Oversight doctrine governs whether directors breached fiduciary duties by failing to implement or monitor board-level reporting systems. "
            "Taken together, demand-futility doctrine addresses board capacity to consider a demand, whereas oversight doctrine addresses the underlying fiduciary breach."
        )

    if target_set == {"demand_futility"}:
        return (
            "Aronson frames demand futility by asking whether particularized facts create a reasonable doubt that directors were disinterested and independent or that the challenged transaction was a valid exercise of business judgment. "
            "Rales refines the inquiry by asking whether particularized facts create a reasonable doubt that a majority of the board could impartially consider a demand. "
            "Taken together, Aronson and Rales define demand futility by focusing on whether demand should be excused."
        )
    ...
    return (
        "The identified doctrines govern distinct fiduciary settings under Delaware law. "
        "Each doctrine applies a different legal standard depending on the board conduct and transactional context involved. "
        "Taken together, the doctrines supply separate analytical frameworks for evaluating fiduciary conduct."
    )
...
def synthesize_multi_doctrine_analysis(target_lines: List[str]) -> str:
    ...
    if target_set == {"oversight"}:
        return (
        "This matters because Caremark establishes the baseline oversight framework requiring an utter failure to attempt to assure a reporting or information system exists. "
        "The significance is that Stone clarifies that a conscious failure to monitor constitutes bad faith and Marchand further requires directors to make a good faith effort to implement and monitor systems addressing mission-critical risk. "
        "As a result, ignoring red flags tied to mission-critical risk may support Caremark liability."
    )
...
def synthesize_structured_single_doctrine_analysis(target_lines: List[str]) -> str:
    ...
    if line == "oversight":
        return (
        "This matters because Caremark requires an utter failure to attempt to assure that a reasonable reporting or information system exists. "
        "The significance is that Stone treats a failure to act in good faith as a duty-of-loyalty problem, while Marchand requires a good faith effort to implement and monitor an oversight system. "
        "As a result, oversight doctrine turns on whether directors acted in bad faith by failing to implement or monitor board-level reporting systems."
    )
...
def synthesize_structured_doctrine_section(doctrine_line: str) -> str:
    ...
    if doctrine_line == "disclosure_loyalty":
        ...
        modern = get_rule_text("disclosure_loyalty", "modern_application")
        return (
            f"{label} doctrine governs what directors owe when communicating truthfully with stockholders, where {foundation}. "
