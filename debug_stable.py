...
def infer_query_type(question: str) -> str:
    q = (question or "").lower()
    if any(
        term in q for term in ["compare", "versus", " vs ", "distinguish", "difference"]
    ):
        return "comparison"
    if any(
        term in q
    ...
    if any(
        term in q
        for term in ["standard", "test", "must plead", "must show", "rule applies"]
    ):
...
def pick_best_quote(quotes: List[str], source: str = "") -> str:
    ...
    def is_garbage(q: str) -> bool:
        ...
        if any(
            ...
            return True
        if "\n" in q and (
            "caremark" in source_l or "stone" in source_l or "kahn" in source_l
        ):
            return True
        return False
...
def build_role_based_quote_map(
    ...
    for role in [
        ...
        for case in cases:
            ...
            best_quote = (
                pick_best_quote(filtered_quotes, source) if filtered_quotes else ""
            )

            print(
                f"DEBUG PICKED QUOTE FOR {source}: {best_quote[:220] if best_quote else ''}"
            )
...
def build_required_quote_fragments_block(role_quote_map, target_lines):
    ...
    if not lines:
        if "controller_transactions" in target_lines:
            lines.append(
                '- MFW: "business judgment review applies if conditioned from the outset"'
            )
...
def synthesize_multi_doctrine_short_answer(target_lines: List[str]) -> str:
    labels = [
        DOCTRINE_LABELS.get(line, line.replace("_", " ").title())
...
def synthesize_rule_from_quotes(
    ...
    foundation_frag = (
        normalize_quote_fragment(
            "foundation", role_quote_map.get("foundation", {}).get("quote", "")
        )
        or "utter failure to attempt to assure"
    )
    refinement_frag = (
        normalize_quote_fragment(
            "supreme_refinement",
            role_quote_map.get("supreme_refinement", {}).get("quote", ""),
        )
    ...
    modern_frag = (
        normalize_quote_fragment(
            "modern_application",
            role_quote_map.get("modern_application", {}).get("quote", ""),
        )
        or "good faith effort to implement an oversight system"
    )
...
def synthesize_doctrine_section(
    ...
    if doctrine_line == "oversight":
        return hard_lock_rule(
            polish_synthesized_rule(
                compress_rule(
                    synthesize_rule_from_quotes(role_quote_map, ["oversight"])
                )
            )
        )
...
def synthesize_short_answer(
    ...
    if "oversight" in target_lines:
        ...
        if primary_failure == "utter_failure":
            return (
                "The theory sounds in implementation failure, not monitoring failure."
            )
        if primary_failure == "failure_to_monitor":
            return (
                "The theory sounds in monitoring failure, not implementation failure."
            )
        ...
        if outcome == "lower_oversight_risk":
            return (
                "The facts suggest lower oversight risk, not a likely Caremark breach."
            )
        return "Caremark requires utter failure, whereas Marchand requires good faith monitoring."
...
def extract_sections(ai_answer: str, query_plan: Dict[str, Any]) -> Dict[str, str]:
    ...
    def section_body(name: str, following: List[str]) -> str:
        if following:
            pattern = (
                rf"{name}\s*:?\s*(.*?)(?="
                + "|".join(rf"\b{x}\b\s*:?" for x in following)
                + r"|$)"
            )
        else:
            pattern = rf"{name}\s*:?\s*(.*)$"
...
def main() -> None:
    ...
    role_quote_map = build_role_based_quote_map(cases, case_quotes)
    ...
    if query_plan.get("multi_doctrine"):
        ...
        locked_rule_a = synthesize_doctrine_section(
            line_a, doctrine_a_cases, case_quotes
        )
        locked_rule_b = synthesize_doctrine_section(
            line_b, doctrine_b_cases, case_quotes
        )
            ...
            enforce_three_sentences(
                compress_rule_comparison(
                    synthesize_rule_comparison(
                        role_quote_map=role_quote_map,
                        tree_result=comparison_tree_result,
                    )
                )
            )
        )
...
""".strip()
    ...
    if query_plan.get("multi_doctrine"):
        ...
        ai_answer = relock_answer_sections(
            ai_answer=ai_answer,
            locked_short_answer=locked_short_answer,
            locked_key_distinction=locked_key_distinction,
            locked_rule=locked_rule,
            locked_rule_comparison=locked_rule_comparison,
